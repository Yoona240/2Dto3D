...
- 检查方法由 `config.yaml` 中的 `edit_quality_check.method` 决定：
  ...
  - `two_stage_recon`（Method-2）：
    - **View Sanity Check（新 Stage 1A）**：先用 before/after 六视图拼图检查 after 图跨视角几何一致性；fail 直接退出，不再执行后续步骤。受 `edit_quality_check.two_stage_recon.view_sanity_check.enabled` 控制。
...
- Stage2 LPIPS backbone 由 `edit_quality_check.two_stage_recon.lpips_net` 控制；当前实现固定为 `alex`。
- 运行环境需显式安装 `lpips` 依赖；若缺失，Stage2 会直接 Fail Loudly 报错。
- 若运行环境缺少 `lpips` 依赖或其预训练 backbone 不可用，Stage2 会直接报错，不做静默降级。
...
**并发日志定位规则：**
- object 级日志统一带 `source_model_id=<12位随机ID>`；同一个 source object 的 `source_t2i / source_gen3d / source_render / instruction_generation` 都应通过这个 ID 串起来。
- edit 级日志统一带：
  - `source_model_id`
  - `edit_id`
  - `edit_scope_id=<source_model_id>_edited_<edit_id>`
- 其中 `edit_scope_id` 是日志/排障专用标识；已有的数据与目录主键仍保持 `target_model_id=<source_model_id>_edit_<edit_id>` 不变。
- 当多个 object 并发执行时，建议直接按 `source_model_id=` 或 `edit_scope_id=` grep `logs/*.log`，不要只靠 stage 名称人工追踪。

**失败与重试日志：**
- `run_full_experiment.py` 现在除了 `[Timing][START/END]` 之外，还会输出：
  - `[Stage][FAIL]`
  - `[Stage][RETRY]`
  - `[Stage][ABORT]`
- 这些行会显式包含：
  ...
  - `attempt`
  - `lane`
  - `source_model_id` / `edit_scope_id`
  - `error_class`
  - `error_type`
  - `message`
  - `response_context` 摘要
- 因此像 `instruction_generation` 这类以前只有 `Timing failed` 的阶段，现在可以直接从主 log 判断是：
  - API 请求失败
  - 响应不是合法 JSON
  - schema 校验失败
  - multiview-safe 规则校验失败
  - 还是质量检查类终态失败

**Hunyuan 下载代理诊断脚本：**
- 当你已经拿到一个具体的 Hunyuan 下载 URL，但不确定失败点到底在代理、首包、重定向还是下载中途卡住时，可以直接运行：

```bash
python tests/test_hunyuan_download_proxy.py \
  --download-url "https://example.com/path/to/model.glb" \
  --max-bytes 10485760
```

- 该脚本默认读取 `config.download_proxy`，并显式使用 `httpx` 代理 transport 复现生产下载路径。
- 如需覆盖代理，可额外传入 `--proxy http://localhost:61234`。
- 如需在“还没收到响应头”或“下载中途长时间没新数据”时持续看到活跃日志，可传 `--heartbeat-seconds 5`。
- 如需把实际响应内容落盘，可传 `--output-file tests/logs/hunyuan_probe.glb`。
- 结果 JSON 默认写到项目根目录 `logs/`，包含 `configured_download_proxy`、`download_proxy`、`status_code`、`first_byte_seconds`、`bytes_downloaded`、`redirect_count`、`final_url`、`error_type`、`error_message` 等字段，适合直接排查下载卡死或代理不通。
...
页面顶部的 **Quick Tools** 区块提供三个快速入口（tab 切换）：

**Tab 1 — Balanced Generate（均衡快速生成）：**

1. 填写 Plan Name、Total Objects（总数量）、Edits per Object（每个 object 的编辑指令数）
2. 选择 Source/Target Provider、Edit Mode、Allowed Types、GPU ID
3. 表单下方会**实时预览**各 category 的 object 分配结果（无需提交，纯前端计算）
4. 点击 “Generate Balanced Plan” 自动生成 YAML 并显示 CLI 命令

系统会将 `total_objects` 均衡分配到全部 19 个 category，分配算法为：
- `base = total_objects // 19`，余数按 pool 大小降序补到最大的 category
- 每个 category 的分配数量不超过其实际 pool 上限（最小 category：Medical & Health 8 个，Lighting & Ceiling 9 个）

**Tab 2 — Get Plan CLI（获取已有 YAML 的执行命令）：**

1. 从历史 YAML 列表选择一个已有 plan
2. 输入目标 GPU ID
3. 点击 “Get Execute CLI” 获取可复制的 `nohup` 命令，**不触发任何后台任务**

**Tab 3 — Get Resume CLI（获取断点继续命令）：**

1. 从列表选择一个未完成的实验（仅显示 `status=running` 或 `interrupted` 的实验）
2. 输入 GPU ID（可覆盖原始值）
3. 点击 “Get Resume CLI” 获取 `--resume-experiment-id` 形式的 `nohup` 命令，**不触发任何后台任务**

以上三个 tab 的输出（YAML 内容 + CLI 命令）均显示在页面右侧的 YAML Preview / CLI Command 区，可一键复制后在服务器终端执行。

**手动配置模式（原有流程，保持不变）：**

1. 访问 `/batch-generation` 页面，向下滚动到 “Existing YAML Plans” 以下区域
